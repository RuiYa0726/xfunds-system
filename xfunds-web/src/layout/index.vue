...
import ProductRecommender from '@/components/ProductRecommender.vue'
...
<template>
  <div class="layout-container">
    ...
    <div class="layout-body">
      <!-- 左侧菜单栏 -->
      <aside class="layout-aside" :class="{ 'aside-hidden': appStore.hidden, 'aside-collapsed': appStore.collapsed && !appStore.hidden }">
        <div class="aside-menu-wrap">
          <SideMenu />
        </div>
        <div v-if="!appStore.hidden" class="aside-actions-bar">
          <div class="action-btn" @click="appStore.toggleCollapse" :title="appStore.collapsed ? '展开菜单' : '折叠菜单'">
            ...
            <span v-if="!appStore.collapsed">{{ appStore.collapsed ? '展开' : '折叠' }}</span>
          </div>
          <div class="action-btn" @click="appStore.toggleHidden" title="收起菜单">
            <el-icon><ArrowLeft /></el-icon>
            <span v-if="!appStore.collapsed">收起</span>
          </div>
        </div>
      </aside>

      <!-- 菜单收起时显示的拉出按钮 -->
      <div v-if="appStore.hidden" class="aside-show-btn" @click="appStore.toggleHidden" title="显示菜单">
  ...
  <MenuNavigator :visible="showNavigator" @close="showNavigator = false" />
  <ProductRecommender />
</template>
...
.aside-actions-bar .action-btn:hover {
  background: #e8ecf1;
  color: #1a2a6c;
}
