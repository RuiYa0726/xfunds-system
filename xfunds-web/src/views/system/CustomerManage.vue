...
// 企业类型下拉选项（V2文档2.4节经营模式划分）
const corpTypeOptions = [
  { value: 'MANUFACTURING', label: '制造业' },
  { value: 'TRADING', label: '贸易业' },
  { value: 'SERVICE', label: '服务业' },
  { value: 'INVESTMENT_HOLDING', label: '投资控股' },
...
function formatCorpType(type) {
  if (!type) return '-'
  const opt = corpTypeOptions.find(o => o.value === type)
...
function openEditDialog(row) {
  ...
  editForm.corpType = row.corpType || ''
    ...
    <el-card shadow="never">
      ...
      >
        ...
        <el-table-column label="企业类型" width="120">
          <template #default="{ row }">{{ row.customerType === 'CORP' ? formatCorpType(row.corpType) : '-' }}</template>
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12" v-if="editForm.customerType === 'CORP'">
            <el-form-item label="企业类型">
              <el-select v-model="editForm.corpType" placeholder="请选择企业类型" style="width: 100%" clearable>
                <el-option
                  v-for="opt in corpTypeOptions"
                  :key="opt.value"
                  :label="opt.label"
                  :value="opt.value"
                />
        ...
        <el-row :gutter="20" v-if="editForm.customerType === 'CORP'">
          <el-col :span="12">
            <el-form-item label="联系电话">
              <el-input v-model="editForm.contactPhone" placeholder="请输入联系电话" />
            </el-form-item>
    ...
    >
      <div v-loading="accountLoading">
        ...
        <el-table :data="accountList" border stripe size="small" style="width: 100%">
          ...
          <el-table-column prop="cnyBalance" label="折人民币余额" width="160" />
