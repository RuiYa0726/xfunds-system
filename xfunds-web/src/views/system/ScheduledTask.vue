...
// 当前查看的任务：null=列表页，'MATURITY_SETTLEMENT'=到期交割详情，'QUOTE_REFRESH'=牌价刷新详情
...
async function loadInfo() {
  ...
  try {
    const fetcher = currentJob.value === 'MATURITY_SETTLEMENT' ? getMaturitySettlementInfo : getQuoteRefreshInfo
...
async function handleRun() {
  ...
  running.value = true
  try {
    const res = isMaturity ? await runMaturitySettlement() : await runQuoteRefresh()
    const d = res.data || {}
    ElMessage.success(
      isMaturity
          ...
          <div class="card-header">
            <div class="header-left">
              ...
              <span class="page-title">
                {{ currentJob === 'MATURITY_SETTLEMENT' ? '到期交割定时任务' : '获取牌价定时任务' }}
              </span>
            </div>
            <el-button
              type="primary"
