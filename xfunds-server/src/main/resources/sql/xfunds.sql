...
CREATE TABLE fx_customer (
    ...
    corp_type      VARCHAR(30)           COMMENT '企业类型（经营模式划分）：MANUFACTURING制造业/TRADING贸易业/SERVICE服务业/INVESTMENT_HOLDING投资控股/FINANCIAL_INSTITUTION金融机构/SME中小民企',
    id_type        VARCHAR(20)           COMMENT '证件类型',
    id_no          VARCHAR(50)           COMMENT '证件号码',
    org_code       VARCHAR(20)           COMMENT '所属机构编码',
    credit_level   VARCHAR(20)           COMMENT '信用等级',
    risk_level     VARCHAR(2)            COMMENT '风评等级：A激进型 B平衡型 C保守型',
    status         VARCHAR(2)   DEFAULT '1' COMMENT '状态：1启用 0停用',
...
CREATE TABLE fx_customer_account (
    ...
    cny_balance   DECIMAL(20,2) DEFAULT 0 COMMENT '账户折人民币余额（按当日即期汇率折算）',
...
CREATE TABLE fx_quote (
    ...
    market_mid_rate    DECIMAL(18,4)          COMMENT '市场中间价',
    total_buy_rate     DECIMAL(18,4)          COMMENT '总行买入价',
    total_sell_rate    DECIMAL(18,4)          COMMENT '总行卖出价',
    branch_customer_buy_rate  DECIMAL(18,4)  COMMENT '分/客买价（分行买入价=客户卖出价）',
    branch_customer_sell_rate DECIMAL(18,4)  COMMENT '分/客卖价（分行卖出价=客户买入价）',
    forward_point      DECIMAL(18,4)          COMMENT '远期点',
    swap_point         DECIMAL(18,4)          COMMENT '掉期点',
...
CREATE TABLE fx_trade_master (
    ...
    spot_rate           DECIMAL(18,4)          COMMENT '即期汇率',
    customer_rate       DECIMAL(18,4)          COMMENT '客户汇率',
    cost_rate           DECIMAL(18,4)          COMMENT '成本汇率',
    branch_profit_point DECIMAL(18,2)          COMMENT '分行利润点',
...
CREATE TABLE fx_forward_trade (
    trade_id              VARCHAR(50)   NOT NULL COMMENT '交易ID',
    maturity_date         DATE                   COMMENT '到期日',
    forward_rate          DECIMAL(18,4)          COMMENT '远期汇率',
    forward_point         DECIMAL(18,4)          COMMENT '远期点',
    settlement_method     VARCHAR(10)            COMMENT '交割方式：FULL/NET',
...
CREATE TABLE fx_swap_trade (
    ...
    near_leg_rate     DECIMAL(18,4)          COMMENT '近端汇率',
    near_leg_cost_rate DECIMAL(18,4)         COMMENT '近端成本汇率',
    near_leg_customer_rate DECIMAL(18,4)     COMMENT '近端客户汇率',
    near_leg_branch_profit_point DECIMAL(18,2) COMMENT '近端分行收益点',
    ...
    far_leg_rate      DECIMAL(18,4)          COMMENT '远端汇率',
    far_leg_cost_rate DECIMAL(18,4)          COMMENT '远端成本汇率',
    far_leg_customer_rate DECIMAL(18,4)      COMMENT '远端客户汇率',
    far_leg_branch_profit_point DECIMAL(18,2) COMMENT '远端分行收益点',
    ...
    swap_point        DECIMAL(18,4)          COMMENT '掉期点',
    near_spot_rate    DECIMAL(18,4)          COMMENT '近端即期汇率',
