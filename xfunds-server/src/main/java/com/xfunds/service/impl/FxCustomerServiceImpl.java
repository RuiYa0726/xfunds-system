...
public class FxCustomerServiceImpl implements FxCustomerService {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(FxCustomerServiceImpl.class);
    ...
    public void saveCustomer(CustomerSaveRequest request) {
        ...
        customer.setCorpType(request.getCorpType());
    ...
    public void addAccount(String customerId, CustomerAccountRequest request) {
        ...
        // 依据当日即期汇率自动赋值折人民币余额
        account.setCnyBalance(calcCnyBalance(request.getCurrency(), account.getBalance()));
    ...
    public void updateAccount(String customerId, Long accountId, CustomerAccountRequest request) {
        ...
        // 余额变动时依据当日即期汇率重算折人民币余额
        account.setCnyBalance(calcCnyBalance(request.getCurrency(), request.getBalance()));
     ...
     * CNY 账户 cny_balance=balance，其他币种 cny_balance=balance*当日X/CNY中间价
     */
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int refreshAllAccountsCnyBalance() {
        List<FxCustomerAccount> accounts = fxCustomerAccountMapper.selectAll();
        if (accounts == null || accounts.isEmpty()) {
            log.info("[折人民币余额刷新] 无客户账户需要更新");
            return 0;
        }
        int updated = 0;
        for (FxCustomerAccount account : accounts) {
            BigDecimal newCnyBalance = calcCnyBalance(account.getCurrency(), account.getBalance());
            if (newCnyBalance != null) {
                account.setCnyBalance(newCnyBalance);
                fxCustomerAccountMapper.update(account);
                updated++;
            } else {
                log.warn("[折人民币余额刷新] 账户 {} 币种 {} 无当日即期汇率，跳过更新",
                        account.getAccountNo(), account.getCurrency());
            }
        }
        log.info("[折人民币余额刷新] 共更新 {} 个客户账户的折人民币余额", updated);
        return updated;
    }
     ...
     * 计算账户折人民币余额
     * CNY 账户：折人民币余额 = 余额
     * 其他币种：折人民币余额 = 余额 × 当日即期中间价（X/CNY）
     * 无对应汇率时返回 null
     */
    private BigDecimal calcCnyBalance(String currency, BigDecimal balance) {
        if (balance == null) {
            balance = BigDecimal.ZERO;
        }
        BigDecimal rate = fxQuoteService.getSpotMidRateToCny(currency);
        if (rate == null) {
            return null;
        }
